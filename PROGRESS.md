...
`0015_custom_orders`). **Тесты:** `pytest` зелёный — 26 (юнит бизнес-логики +
сквозные smoke-флоу: регистрация→магазин→товар→корзина→заказ). **Сборка:**
backend импортируется во всех 5 сервисах, frontend `vite build` проходит.

---

## Прод-подготовка + UX-волна (30.06–01.07) [DONE]

**Наблюдаемость и эксплуатация**
- Prometheus + Grafana на бастионе (группа `[monitoring]` генерируется Terraform);
  дашборд «Marketplace» встроен в админку (Метрики, `grafana_dashboard_url`).
- Централизованные логи: **Alloy** на каждой ноде → **Loki** (приватный IP бастиона,
  ретеншн 7 дн.) → Grafana (дашборд «Логи (Loki)», Explore).
- `infra/scripts/project-power.sh stop|start|status` — пауза всего проекта (PG, Redis,
  инстанс-группы, бастион) без потери данных и IP сайта.

**Хранилище**
- Бакеты разделены: публичный (картинки/медиа) и **приватный** (цифровые товары,
  HLS, KYC — только presigned; анонимный GET = 403). `S3_PRIVATE_BUCKET` с фолбэком.

**Отказоустойчивость и безопасность (этап 1 прод-плана)**
- Kong: upstream'ы со **всеми** backend-нодами + активные health-check `/health`
  (раньше весь трафик шёл на первую ноду). То же для catch-all фронтенда.
- Security Groups сужены до подсетей VPC (сервисные порты, Kong admin, SSH);
  у бастиона появилась своя SG (Prometheus/Loki закрыты от интернета).
- Celery: авторетраи с экспоненциальной задержкой + DLQ `celery:dlq` в Redis.
- Бэкапы PG: managed 7 дней (явно в Terraform) + ночной offsite `pg_dump` в
  приватный бакет (cron). Rate-limit на всех auth-ручках (регистрация, коды, сброс).
- Найдено и починено smoke-тестами: 500 на auth при недоступном Redis (slowapi
  без in-memory fallback), MissingGreenlet в create/update товара (ленивая
  загрузка images при сериализации), дубликат индексов FiscalReceipt.

**Поиск**
- MeiliSearch (контейнер на backend-ноде, ключ из Terraform, порт только в VPC),
  почасовая переиндексация Celery; `GET /products/suggest`; фронт: живые подсказки
  в шапке (товары с фото и ценой, категории, магазины).

**Витрина / UX**
- Мобильная версия: бургер-меню (Drawer), фильтры каталога в Drawer, сетка 2 колонки.
- **Гостевая корзина** (localStorage) со слиянием при входе; `/cart` публичный.
- Единая янтарная палитра (убран разнобой оранжевых), общий `ProductCard`
  (избранное-сердечко, бейдж скидки −N%, звёзды), skeleton-загрузка, debounce
  слайдера цены, EN-переключатель скрыт до полного перевода.
- Футер с колонками + инфостраницы `/info/*`: о нас, контакты, FAQ, доставка и
  оплата, оферта, политика (152-ФЗ), пользсоглашение (юртексты — шаблоны,
  реквизиты заполнить перед запуском).
- SEO: `react-helmet-async` — title/description/OG на товар (с картинкой),
  категорию, магазин, главную; нормальный `index.html`.
