...
- **Ansible** ([infra/ansible/deploy-services.yml](infra/ansible/deploy-services.yml))
  ...
  + MeiliSearch (на первой ноде), гоняет миграции и настраивает Kong.
- **HA через Kong upstreams:** на каждый сервис создаётся upstream, в таргетах —
  **обе** backend-ноды с активным health-check по `/health` (упавшая нода выводится
  из ротации автоматически). Catch-all фронтенда балансируется так же
  (`frontend-upstream` на `:3000` обеих нод).
- **Security Groups сужены:** сервисные порты (8001–8005, 3000), Kong admin `:8001`,
  MeiliSearch `:7700` и node-exporter `:9100` доступны **только из подсетей VPC**;
  SSH — только из подсети бастиона; публично открыты лишь Kong `:8000` (вход сайта),
  SSH бастиона и Grafana `:3000` (iframe в админке).
- **Managed-дополнения:** `storage.tf` — **два** Object Storage бакета
  (публичный: картинки/медиа; приватный: цифровые товары/HLS/KYC + pg-дампы),
  `mail.tf` — **Postbox** (SMTP), `search.tf` — ключ MeiliSearch. **Фронтенд**
  деплоится отдельным образом/плейбуком ([deploy-frontend.yml](infra/ansible/deploy-frontend.yml))
  за тем же Kong. **Наблюдаемость** (Prometheus+Grafana+Loki+Alloy) — плейбук
  [observability.yml](infra/ansible/observability.yml), раздел 9a. Сборка/пуш всех
  образов — [build-and-push.sh](build-and-push.sh) или GitHub Actions.
- **Надёжность фоновых задач:** все Celery-задачи ретраятся с экспоненциальной
  задержкой (до 5 попыток); исчерпавшие попытки попадают в Redis-список
  `celery:dlq` для ручного разбора.
- **Тесты:** `shared/tests` — юнит + сквозные smoke-флоу (регистрация→магазин→
  товар→корзина→заказ) на in-memory SQLite, без внешних сервисов; гоняются в CI.
...
## 9-bis. Кабинет продавца на поддомене (`seller.<домен>`)

Кабинет продавца вынесен на **отдельный вход на поддомене** `seller.<домен>` со своим
sider-меню (как в админке). Технически это **тот же** фронтенд-контейнер и тот же бандл:
на старте SPA смотрит на `window.location.hostname` и, если он начинается с `seller.`,
рендерит отдельное приложение продавца (`src/seller/SellerApp.tsx`: свой логин +
`SellerLayout`) вместо витрины. Маршруты внутри кабинета — корневые: `seller.<домен>/`,
...
поднимется сразу в режиме кабинета продавца.

**На будущее (Вариант 2):** код намеренно изолирован (`SellerApp` — самостоятельное
дерево роутов), так что переезд на отдельный Vite-entry / отдельный контейнер с
собственным Kong-роутом по хосту будет правкой сборки/деплоя, а не переписыванием.

---

## 9a. Наблюдаемость (Prometheus + Grafana + Loki + Alloy)

```bash
# группа [monitoring] (= бастион) генерируется Terraform-ом автоматически
cd infra/terraform
ansible-playbook ../ansible/observability.yml
```

[observability.yml](infra/ansible/observability.yml) поднимает **метрики и логи**:

- **Prometheus** (бастион `:9090`, доступ только из VPC) — скрейпит `/metrics`
  сервисов (8001–8005), Kong и node_exporter'ы (`:9100`).
- **Grafana** (бастион `:3000`, публично — на ней держится iframe в админке;
  анонимный доступ только Viewer) с провиженнутыми дашбордами: **Marketplace**
  (метрики: rps, p95, 5xx) и **Логи (Loki)**. В админке (раздел **Метрики**)
  вставьте URL дашборда (настройка `grafana_dashboard_url`) — kiosk-режим:
  `http://<бастион>:3000/d/marketplace/marketplace?kiosk&theme=light`.
- **Loki** (бастион `:3100`, только приватный IP) + **Grafana Alloy** на каждой
  ноде (backend/kong/monitoring) — Alloy читает stdout/stderr всех
  docker-контейнеров и шлёт в Loki (ретеншн 7 дней). Смотреть: Grafana →
  Explore → Loki или дашборд «Логи (Loki)» с фильтром по контейнеру.

Sentry включается переменной `SENTRY_DSN` (по умолчанию выключен).

---

## 9b. Бэкапы PostgreSQL

- **Managed-бэкапы**: включены явно в Terraform — `backup_retain_period_days=7`,
  окно 01:00 (см. [database.tf](infra/terraform/database.tf)).
- **Offsite-дамп**: cron `02:30` на первой backend-ноде запускает
  [pg-backup.sh](infra/ansible/files/pg-backup.sh) — `pg_dump | gzip` →
  **приватный** S3-бакет (`s3://…-private/pg-backups/`). Переживает удаление
  кластера. Устанавливается автоматически в `deploy-services.yml`.

## 9c. Пауза проекта (экономия)

```bash
./infra/scripts/project-power.sh stop|start|status
```

Гасит/поднимает PG, Redis, обе instance-группы и бастион; NLB/бакеты/сеть не
трогает (IP сайта сохраняется). После `start`: публичный IP бастиона меняется →
`terraform apply` (перегенерит hosts.ini) и поправить `grafana_dashboard_url`.
...
# Object Storage — ДВА бакета (опц. — пусто → локальный диск, только для одной ноды):
#   S3_BUCKET         — ПУБЛИЧНЫЙ (картинки товаров/медиа, per-object public-read)
#   S3_PRIVATE_BUCKET — ПРИВАТНЫЙ (цифровые товары, HLS, KYC; только presigned;
#                       пусто → однобакетный режим, всё в S3_BUCKET)
S3_ENDPOINT=https://storage.yandexcloud.net  S3_BUCKET=<pub>  S3_PRIVATE_BUCKET=<priv>  S3_ACCESS_KEY=<k>  S3_SECRET_KEY=<s>  S3_PUBLIC_URL=...
...
# Полнотекстовый поиск (опц. — пусто → поиск по БД ILIKE):
#   MeiliSearch поднимается контейнером на первой backend-ноде (deploy-services.yml);
#   индекс перестраивается Celery-задачей раз в час.
MEILI_URL=http://<backend0>:7700   MEILI_KEY=<из hosts.ini meili_key>
